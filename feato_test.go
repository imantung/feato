...
func TestManager(t *testing.T) {
	...
	toggleRouter := feato.NewMockRouter(ctrl)
	feato.SetToggleRouter(toggleRouter)

	t.Run("GIVEN route return enable", func(t *testing.T) {
		toggleRouter.EXPECT().Route(gomock.Any()).Return(feato.Enabled)
		ok, err := feato.Run(feato.NewFeature("key1"), func() error {
	...
	t.Run("GIVEN route return disable", func(t *testing.T) {
		t.Run("WHEN no disable run function", func(t *testing.T) {
			toggleRouter.EXPECT().Route(gomock.Any()).Return(feato.Disabled)
			ok, err := feato.Run(feato.NewFeature("key2"), func() error {
				return nil
			})
